...
namespace OffsetHoffmann1
{
    ...
    {
        Matrix<double> P = Matrix<double>.Build.Dense(4, 2);
        ...
        private void Canva_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
               // e.Graphics.DrawEllipse(big, MatrixToPointArray(P)[i].X - r, MatrixToPointArray(P)[i].Y - r, 2 * r, 2 * r);
            }
            ...
            for (int i = 0; i < splitValues.Count - 1; i++)
            {
                ...
                e.Graphics.DrawBeziers(Pens.Black, MatrixToPointArray(offset));
            }

            e.Graphics.DrawBeziers(Pens.Green, MatrixToPointArray(P));
        }
        
        private bool IsSafe()
        {
            double maxError = 0.5; //vertiba pikselos
            List<double> newSplitValues = new List<double>();
            ...
            for (int i = 0; i < splitValues.Count - 1; i++)
            {
                ...
                for (double t = 0; t <= 1; t += 0.1)
                {
                    ...
                    {
                        newSplitValues.Add((splitValues[i] + splitValues[i + 1]) / 2);
                        break;
                    }
                }
            }

            if (newSplitValues.Count == 0)
            {
                return true;
            }
            ...
            {
                splitValues.AddRange(newSplitValues);
                splitValues.Sort();
                return false;
            }
        }
